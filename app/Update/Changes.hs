...
prettyPrintChanges Changes {..} = Text.unlines $ catMaybes [newSongs, modifiedSongs, deletedSongs]
  where newSongs, modifiedSongs, deletedSongs :: Maybe Text
        ...
        template _ [] = Nothing
        template prompt items = Just $ Text.pack $ printf "%s: [%s]" prompt (intercalate ", " items)
